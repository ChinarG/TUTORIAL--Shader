Shader"Tutorial/Shader08-高光反射-Blinn-Phong"
{
    Properties
	{
	...
	SubShader
	{
        Pass
		{
			...
			struct v2f
			{
				...
				float4 worldVertex:TEXCOORD1;//世界位置，的，顶点坐标
			...
			v2f vert(a2v v)
			{
				...
				f.worldNormal = UnityObjectToWorldNormal(v.normal);//返回的是，从模型空间 转换到 世界空间的法线

				f.worldVertex = mul(v.vertex,unity_WorldToObject);//顶点坐标转世界坐标
			...
			fixed4 frag(v2f f):SV_Target
			{
				...
				fixed3 lightDir = normalize(WorldSpaceLightDir(f.worldVertex).xyz);//传入顶点坐标，然后返回一个 顶点到光源的光线，然后单位化向量
				...
				fixed3 halfDir = normalize(viewDir+lightDir);//平分线：视野方向，和平行光方向。

				fixed3 specular = _LightColor0.rgb *_Specular.rgb* pow(max(dot(normalDir,halfDir),0),_Gloss);//反射光：Blinn-Phong 法线 - 平分线
